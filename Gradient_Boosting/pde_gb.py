...
from deepxde.utils import get_num_args, run_if_all_none, mpi_scatter_from_rank0
...
class PDE(Data):
    ...
    def losses(self, targets, outputs, loss_fn, inputs, model, aux=None):
        ...
        ##########################################
        # 增加归一化损失函数
        # error_norm = tf.math.reduce_sum(outputs_pde * outputs_pde) - outputs_pde.shape[0]
        # losses.append(loss_fn[0](tf.zeros_like(error_norm), error_norm))
        ############################################
        return losses
